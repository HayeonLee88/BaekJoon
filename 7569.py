...
tips:
        1. 시간 초과: 안 익은 토마토를 덱에 담아 탐색
                    - 안 익은 토마토가 익는다면 ? 만약 당일 익은 토마토 주변에 익지 않은 토마토가 있으면 영향을 미친다.
                    - 위를 해결하기 위해 당일 익은 토마토를 다른 덱에 담고 안 익은 토마토 덱을 모두 탐색한 후, 당일 익은 토마토를 1로 바꾼다.
                     이 과정에서 시간이 초과된다.
           해결 방법: 익은 토마토를 덱에 담아 탐색하기
                    - 익은 토마토와 인접한 안익은 토마토를 1로 변경하고, 이 토마토를 덱에 담아도 익지 않은 다른 토마토에 영향을 미치지 않는다.
'''
import sys
